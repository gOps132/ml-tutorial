...
void data_handler::read_feature_vector(std::string path)
{
    ...
    if (f)
    {
        ...
        printf("Sucessfully read and stored %lu feature vectors", data_array->size());
    }
    else
    {
...
{
    uint32_t header[2];     // [MAGIC_NUM|NUM_IMAGES]
    unsigned char bytes[4]; //32 bits
    FILE *f = fopen(path.c_str(), "r");
    if (f)
    {
        ...
        for (int i = 0; i < header[1]; i++)
        {
            ...
            {
                printf("Read Feature Label Error Reading from file. \n");
                exit(1);
            }
        }
        printf("Sucessfully read and stored label. \n");
    }
    else
    {
        printf("Could not find file \n");
...
void data_handler::split_data()
{
    ...
    printf("training_data size %lu\n", training_data->size());
    printf("test_data size %lu\n", test_data->size());
    printf("validation_data size %lu\n", validation_data->size());
}
...
void data_handler::count_classes()
{
    ...
    num_classes == count;
    printf("Sucessfully esxtracted Unique Classes. \n");
}
